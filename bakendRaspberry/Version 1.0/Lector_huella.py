...
def VerifyUser():
    ...
    r = TxAndRxCmd(command_buf, 8, 5);
    print(g_rx_buf)
    #if g_rx_buf == [245, 12, 0, 0, 5, 0, 9, 245]:
    #    validacion(AddUser())
    if g_rx_buf != [245, 12, 0, 0, 5, 0, 9, 245]:
        validacion(g_rx_buf)
    # nada = []
...
def main():
    ...
    print ("  CMD1 : Consulta el número de huellas registradas")
    print ("  CMD2 : Regisrar huella (ponga el dedo índice derecho en el sensor) ")
    print ("  CMD3 : Verificación de huella  (Send the command, put your finger on sensor) ")
    print ("  CMD4 : Borrar todas los registros de huella")
